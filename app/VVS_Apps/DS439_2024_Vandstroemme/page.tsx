...
const TableComponent: React.FC = () => {
  ...
  const qdBK = 2*0.075 + 0.011*(sumBrugsvandK - 2*0.075) + 3.1*Math.sqrt(0.075*0.011)*Math.sqrt(sumBrugsvandK - 2*0.075);
  //const qdBV = 0.2 + 0.015 * (sumBrugsvandV - 0.2) + 0.12 * Math.sqrt(sumBrugsvandV - 0.2);
  const qdBV = 2*0.075 + 0.011*(sumBrugsvandV - 2*0.075) + 3.1*Math.sqrt(0.075*0.011)*Math.sqrt(sumBrugsvandV - 2*0.075);
    ...
    <main style={{ flex: 1, padding: '0.5%' }}>
      ...
      {showAssumptions && (
        <div className={styles.teoriBox}>
          ...
          q<sub>d</sub> = 2 * 0,075 + 0,011 * (&Sigma;q<sub>f</sub> - 0,2*0,075) + 3,1 * √(0,075*0,11) * √(&Sigma;q<sub>f</sub> - 0,2*0,0075) [l/s]

          <p></p>
          q<sub>f</sub>, er fundet ud fra værdierne i nedestående tabel, fra DS 439:2024 afsnit. 6.3.4 <br />
      ...
      <div className={styles.container}>
        <h3> DS 439:2024 <br /> Dimensionsgivende vandstrøm q<sub>d</sub> <br />for forskellige forudsatte vandstrømme
        q<sub>f</sub></h3>
